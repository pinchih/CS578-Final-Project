...
if [ "$mode" = "model" ];then
	...
	do
		...
		START=$(date +%s)
		echo "dbg: create model for $file"
		#java -Xmx10G -jar covert.jar -mode analyzer2 -in "$file" -out ../"$extractionOutput" -map ../"$domainMap" -map_api ../"$apiMap"
		java -Xmx512M -jar covert.jar -mode analyzer2 -in "$file" -out ../"$extractionOutput" -map ../"$domainMap" -map_api ../"$apiMap"
		END=$(date +%s)
		DIFF=$(( $END - $START ))
...
#Generating DSL from the models
	...
	if [ "$mode" = "dsl" ];then
		...
		java -Xmx512M -jar resources/covert.jar -mode composer -in "$extractionOutput" -out "$alloyResult" -filter n
	else
		#java -Xmx8G -jar resources/covert.jar -mode t_composer2 -in "$mergedOutput" -out "$alloyResult" -templates "$tmpl" -filter "$filter_type"
		java -Xmx512M -jar resources/covert.jar -mode t_composer2 -in "$mergedOutput" -out "$alloyResult" -templates "$tmpl" -filter "$filter_type"
	fi
...
#Generating alloy results
	...
	java -Xmx512M -jar covert.jar -mode solver -in ../"$alloyResult"/ICC.als -out ../"$solverResult"
...
#Generating policies
	...
	java -Xmx512M -jar covert.jar -mode policy -in ../"$solverResult" -out ../../$appRepo
